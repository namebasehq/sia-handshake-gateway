...
function handler(proxy) {
  return async (req, res) => {
    ...
    } else if (hash.startsWith(ipfsPrefixLegacy)) {
      gateway = ipfsGateway;
      rawHash = hash.substring(ipfsPrefixLegacy.length);
    } else {
      gateway = siaGateway;
...
`);
        ...
        proxy.web(req, res, {
          ...
          target: url,
          headers: gateway === siaGateway && siaApiKey.length > 0 ? {
              'Skynet-Api-Key': siaApiKey
          } : {}
        });
      }
    } catch (err) {
