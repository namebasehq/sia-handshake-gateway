...
const { nameserver, hdns } = require('./config');

const { Resolver } = dns.promises;
const nameserverResolver = new Resolver();
nameserverResolver.setServers([nameserver]);

const hdnsResolver = new Resolver();
hdnsResolver.setServers(hdns);
...
async function hdnsResolveTxt(zone) {
  return hdnsResolver.resolveTxt(zone);
}
